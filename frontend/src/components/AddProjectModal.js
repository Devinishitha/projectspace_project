...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    const [isProjectCreated, setIsProjectCreated] = useState(false); // Add state to track if the project is created

    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`http://localhost:9000/api/project/${id}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!edit) {
            axios.post('http://localhost:9000/api/project/', { title, description: desc })
                .then((res) => {
                    setIsProjectCreated(true); // Set the flag to true when project is created
                    closeModal();
            ...
            axios.put(`http://localhost:9000/api/project/${id}`, { title, description: desc })
                .then((res) => {
                    closeModal();
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
            ...
            <Dialog as='div' open={isModalOpen} onClose={() => closeModal()} className="relative z-50">
                <div className="fixed inset-0 overflow-y-auto">
                    ...
                    <div className="fixed inset-0 flex items-center justify-center p-4 w-screen h-screen ">
                        ...
                        >
                            <Dialog.Panel className={`rounded-md ${isProjectCreated ? 'bg-yellow-200' : 'bg-white'} w-6/12`}> {/* Apply yellow background if project is created */}
