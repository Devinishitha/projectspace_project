...
function Task() {
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios
                .get(`http://localhost:9000/api/project/${projectId}`)
                .then((res) => {
                    ...
                    setColumns({
                        [uuid()]: {
                            ...
                            items: res.data[0].task
                                .filter((task) => task.stage === 'Team Members')
                                .sort((a, b) => {
    ...
    const updateTodo = (data) => {
        axios
            .put(`http://localhost:9000/api/project/${projectId}/todo`, data)
            .then((res) => {})
            .catch((error) => {
                toast.error('Something went wrong');
            });
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios
            .delete(`http://localhost:9000/api/project/${projectId}/task/${taskId}`)
            .then((res) => {
    ...
    return (
        <div className='px-12 py-6' style={{ background: 'linear-gradient(to bottom, #FFFACD, #87CEFA)', minHeight: '1000px' }}>
            <div className="flex items-center justify-between mb-6">
                ...
                <div>
                    ...
                    <button className="ml-3 bg-green-500 text-white px-3 py-1 rounded-md" onClick={handlePrint}>Print</button>
                </div>
            </div>
            <DragDropContext
                                        ...
                                        {(provided, snapshot) => {
                                            return (
                                                <div
                                                    ...
                                                    className={`min-h-[830px] pt-4 duration-75 transition-colors border-t-2 border-indigo-400 ${snapshot.isDraggingOver && 'border-indigo-600'}`}
                                                >
